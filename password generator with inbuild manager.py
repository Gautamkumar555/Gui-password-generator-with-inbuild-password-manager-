...
def delete_password():
    service = service_entry.get().strip()
    if not service:
        messagebox.showwarning("Missing Information", "Please enter the service name to delete the password.")
        return
    with open(PASSWORD_FILE, "r") as file:
        data = json.load(file)
    if service in data:
        ...
        with open(PASSWORD_FILE, "w") as file:
            json.dump(data, file, indent=4)
        messagebox.showinfo("Success", f"Password for '{service}' has been deleted.")
        service_entry.delete(0, tk.END)
    else:
...
delete_button = ttk.Button(button_frame, text="Delete Password", command=delete_password)
